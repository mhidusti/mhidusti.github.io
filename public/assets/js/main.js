...
(function ($) {
  ...
  $(document).ready(function ($) {
    ...
    /  Portfolio Filter
    /------------------------------------------------------*/
    if ($(".portfolio-box").length > 0 && typeof $.fn.imagesLoaded === "function") {
      ...
      console.warn("imagesLoaded or .portfolio-box not available - isotope init skipped.");
    }

    /*------------------------------------------------------
    /  Portfolio Gallery Carousel
    /------------------------------------------------------*/
    if ($.fn.owlCarousel && $(".portfolio_gallery.owl-carousel").length > 0) {
    ...
    / Testimonial Carousel
    /------------------------------------------------------*/
    if ($.fn.owlCarousel && $(".testimonial-carousel.owl-carousel").length > 0) {
    ...
    /------------------------------------------------------*/
    if ($.fn.owlCarousel && $(".tj-post__gallery.owl-carousel").length > 0) {
    ...
    try {
      if (typeof Swiper !== "undefined" && $(".brand-slider").length > 0) {
        ...
        console.warn("Swiper not loaded or .brand-slider not found.");
      }
    } catch (err) {
      console.warn("Error initializing Swiper:", err);
    }
    ...
    /*------------------------------------------------------
    /  ALL Popup (lightcase / magnific)
    /------------------------------------------------------*/
    try {
      if ($(".popup_video").length > 0 && typeof $.fn.lightcase === "function") {
      ...
      console.warn("lightcase init failed", e);
    }

    if ($.fn.magnificPopup && $(".modal-popup").length > 0) {
  ...
  /*------------------------------------------------------
  /  Window load (animations, preloader, etc.)
  /------------------------------------------------------*/
  $(window).on("load", function () {
    ...
    /*------------------------------------------------------
    /  Preloader (GSAP) - guarded
    /------------------------------------------------------*/
    try {
      var svg = document.getElementById("preloaderSvg");
      var svgText = document.querySelector(".preloader-heading .load-text, .preloader-heading .cont");
      ...
      if (typeof gsap !== "undefined" && svg) {
        ...
        if (svgText) {
          // If there is an inner svg <text> for stroke animation, add class
          try {
            svgText.classList.add("animate-stroke");
          } catch (e) {
            // svgText might be a NodeList or different node; ignore if fails
          }
        } else {
          console.warn("preloader svg text not found for stroke animation.");
        }
      } else {
        console.warn("gsap or preloader svg not available; skipping preloader timeline.");
        // hide preloader quickly so page content visible
        try {
          $(".preloader").hide();
        } catch (e) {}
      }
    ...
    // Form Validation
    if ($("#contact-form").length > 0 && typeof $.fn.validate === "function") {
      $("#contact-form").validate({
        ...
        submitHandler: function (form) {
          ...
          $.ajax({
            ...
            error: function (err) {
              console.warn("Contact form AJAX error", err);
              $("#message_fail").modal("show");
            },
          });
        },
